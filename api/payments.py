...
def create_checkout_session(
    ...
    try:
        checkout_session = stripe.checkout.Session.create(
            ...
            success_url="http://e1.chiasegpu.vn/success",
            cancel_url="http://e1.chiasegpu.vn/cancel",
        )
...
async def stripe_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        if not metadata and hasattr(session, 'get'):
        ...
        if metadata:
            ...
            if user_id and tokens_to_add:
                ...
                if user:
                    ...
                    # 2. Nâng cấp Pro nếu mua gói nâng cao
                    if package_id == 'pro':
                        user.account_type = 'pro'
                    
                    db.commit()
                    print(f"💰 WEBHOOK: User {user_id} nạp {tokens_to_add} ký tự. Hạng: {user.account_type}")
